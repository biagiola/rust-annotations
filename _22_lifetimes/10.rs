...
// lifetime, a non-specific lifetime, a dynamic future lifetime that can vary
// . We can annotate generic lifetimes in our code. This enables
// functions be are flexible enough to handle varying lifetimes.
...
// . A lifetime annotation is a piece of metadata, more infomation about the
// variable o function that we provide to the borrow checker so it can validate
...
// We are explicitly annotating what Rust inferred in the previous lesson thanks
// to one of many Lifetimes elision rules that we'll disscuss in more details later.

// keep it in mind that, the connection is not really between the return
