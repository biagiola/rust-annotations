...
fn main() {
    ...
    // we can explicity annotate its return types like this:
    ...
    // it will inferred a unit but in closure it inferres base
    // on the body of it.
    let multiply_by = |value| { value * multipler() };
    // also, rust will inferred the parameter type by first time invocation
    // println!("{}", multiply_by(2 as u8));
    println!("{}", multiply_by(2)); // i32
    // after first invocation, the param type will persist
